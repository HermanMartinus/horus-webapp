...
function createPassphrase() {
  const seed = document.querySelector('#seed').value;
  const service = document.querySelector('#service').value;
  document.querySelector('#passphrase').value = generatePassphrase(seed, service)
...
function generatePassphrase(seed, service) {
  var combo = (service + seed).toLowerCase();
  var hashObject = MD5(combo);
  var digitList = hashObject.replace(/\D/g,'').match(/.{1,4}/g);
  var words = [];
